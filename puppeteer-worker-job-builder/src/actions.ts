...
export type CreateActionFunction = (...params: any[]) => Action;
...
export class For extends Action {
  generator: any[] | ArrayGeneratorFunction | Action;
...
export class GetParamsValueByPath extends Action {
  ...
  async run() {
    const value = _.get(this.currentContext.params, this.path);
    return value;
  }
}
...
export class IsTwoValueEqual extends Action {
  ...
  constructor(value: any, otherValue: any) {
...
export class IsActionOutputEqualValue extends Action {
  ...
  constructor(action: Action, value: any) {
    super(IsTwoValueEqual.name);
    this.action = action;
    this.value = value;
  }
...
export class SetVarsWithActionOutput extends Action {
  ...
  async run() {
    const output = await runNestedAction(this, this.action);
    _.set(this.currentContext.vars, this.path, output);
    return this.currentContext.vars;
  }
}
...
export class SetVarsDirectValue extends Action {
  ...
  async run() {
    _.set(this.currentContext.vars, this.path, this.value);
    return this.currentContext.vars;
  }
}
...
export class GetVars extends Action {
  ...
  async run() {
    const output = await _.get(this.currentContext.vars, this.path);
    return output;
  }
}
